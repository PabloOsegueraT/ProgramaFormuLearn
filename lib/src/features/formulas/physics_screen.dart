...
class _PhysicsScreenState extends State<PhysicsScreen> {
  ...
  Map<String, String> _asStringMap(dynamic v) {
    ...
    // String: "v: Velocidad (m/s), d: Distancia (m), t: Tiempo (s)"
    final out = <String, String>{};
  ...
  /// Convierte el doc a un modelo simple para la UI.
  _Phys _fromDoc(DocumentSnapshot<Map<String, dynamic>> d) {
    final data = d.data() ?? {};
    ...
    // ✅ variables desde Firestore (map/list/string → Map<String,String>)
    final vars = _asStringMap(data['variables']);

    // condiciones_uso puede venir como String con viñetas o como List
    final conds = _asStringList(data['condiciones_uso']);
    ...
    return _Phys(
      ...
      explanation: desc,
      topic: topic,
      variables: vars,       // 👈 ahora sí enviamos variables
      conditions: conds,
    );
  }

  bool _isPhysics(String s) {
    final t = s.toLowerCase();
    return t.contains('física') || t.contains('fisica');
  }
            ...
            children: [
              TextField(
                ...
                decoration: InputDecoration(
                  hintText: 'Buscar fórmula (p.ej. “MRU”, “Newton”, “K=…”)',
